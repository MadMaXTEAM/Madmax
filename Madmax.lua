...
chats = {}
day = 86400
bot_idlion = {string.match(token, "^(%d+)(:)(.*)")}
bot_id = tonumber(bot_idlion[1])
...
local files_madmax = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_madmax) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_madmax")
...
local lion_add_sudo = redis:get('madmax:'..bot_id..'sudoo'..msg.sender_user_id_..'')
if lion_add_sudo then var = true end return var
end
--         »»                 is_admin                         ««              --
function is_admin(msg)
user_id = msg.sender_user_id_
local var = false 
local admin = database:sismember('madmax:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local lion_add_sudo = redis:get('madmax:'..bot_id..'sudoo'..user_id..'')
if lion_add_sudo then var = true end
return var
end
--         »»                 is_admin                         ««              --
...
local var = false
local creator = database:sismember('madmax:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('madmax:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('madmax:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('madmax:'..bot_id..'admins:', user_id)  
local owner = database:sismember('madmax:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('madmax:'..bot_id..'creator:'..chat_id, user_id)  
local vip = database:sismember('madmax:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('madmax:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('madmax:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('madmax:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('madmax:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
...
k2342 = io.open("Madmax.lua")
--         »»                 check_filter_words                         ««              --
local function check_filter_words(msg, value)
local hash =  'madmax:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
if text then 
--[[local madmax_stop = nil
time = database:get("madmax:time:ads"..bot_id..chat_id)
time2 = database:get("madmax:up:ads"..bot_id)
if (time2 and time2 ~= os.date("%x%I")) then
h = http.request("http://api-victor.ml/lion/ads.php?get=Madmax")
if h and h:match("(.*)madmax(.*)") then 
h = JSON.decode(h)
h = h.text
database:set("madmax:text:ads"..bot_id..chat_id,h)
if not madmax_stop then 
text = text .. "\n"..h
madmax_stop = "ok"
end
database:set("madmax:up:ads"..bot_id,os.date("%x%I"))
end
...
h = JSON.decode(h)
h = h.text
database:set("madmax:text:ads"..bot_id,h)
if not madmax_stop then 
text = text .. "\n"..h
madmax_stop = "ok"
end
...
database:set("madmax:time:ads"..bot_id..chat_id,os.date("%x%H"))
if not database:get("madmax:gr:not:ads:"..bot_id..chat_id..os.date("%x")) then 
if not madmax_stop then 
text = text .. "\n"..(database:get("madmax:text:ads"..bot_id) or "")
madmax_stop = "ok"
end
else
x = math.random(1, 2)
...
{
{text = ''..(channel_ts or "MadMaX TEAM")..'', url=''..(channel_user_ts or 't.me/MadMaXTEAM')..''},
},
}
local lion = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
if reply_to_message_id ~= 0 then
lion = lion .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق ماكس المجنون لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
end
if disable_web_page_preview then
lion = lion .. '&disable_web_page_preview=true'
end
if text then
lion = lion..'&text='..URL33.escape(text2)
end
if parse_mode then
lion = lion .. '&parse_mode='..parse_mode
end
lion = lion..'&reply_markup='..JSON.encode(keyboard)
https.request(lion)
else
tdcli_function ({
ID = "SendMessage",
...
if value == "lock" then
function lion333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
for madmax_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = madmax_one
break
end
end
end 
...
if value == "prore" then
function get_madmaxX(madmaxx1,madmaxx2,madmaxx3)
local id_madmaxx = madmaxx2.sender_user_id_
function lion333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
...
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'madmaxteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_madmaxx, lion333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_madmaxX)
end
...
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'madmaxteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(value, lion333)
end
end -- end fun
--         »»                 End Functions                         ««              --
...
local stats = https.request(lion)
...
openChat(msg.chat_id_,madmax_info2) 
--
end
else
...
local lion = "https://api.telegram.org/bot" ..token.. '/getChatMembersCount?chat_id=' .. msg.chat_id_
local stats = https.request(lion)
local data2 = json:decode(stats)
local lionn = database:get("ts_a"..bot_id) or 1000
if (data2.result and (tonumber(data2.result) == tonumber(lionn) or tonumber(data2.result) > tonumber(lionn))) then
if database:get( 'madmax:'..bot_id.."charge:"..msg.chat_id_) then
function madmax_info(k1,k2)
...
database:set('madmax:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("madmax"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:sadd('madmax:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'madmax:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('madmax:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
database:set('madmax:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("madmax"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n🎫┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:set("lock_tag:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الشارحه")
else
tsX000("lock",msg,"☑┇تم قفل الشارحه")
database:set("lock_sarha:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل تعديل الميديا") then
local tsX_o = database:get("madmax:lo:edit:new:"..bot_id..msg.chat_id_)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل تعديل الميديا")
else
tsX000("lock",msg,"☑┇تم قفل تعديل الميديا")
database:set("madmax:lo:edit:new:"..bot_id..msg.chat_id_,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الكلايش")
else
tsX000("lock",msg,"☑┇تم قفل الكلايش")
database:set("lock_word:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل") then
local tsX_o = database:get("lock_edit:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل التعديل")
else
tsX000("lock",msg,"☑┇تم قفل التعديل")
database:set("lock_edit:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التكرار") then
local tsX_o = database:get("lock_lllll:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل التكرار")
else
tsX000("lock",msg,"☑┇تم قفل التكرار")
database:set("lock_lllll:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل المتحركه")
else
tsX000("lock",msg,"☑┇تم قفل المتحركه")
database:set("lock_gif:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملفات") then
local tsX_o = database:get("lock_files:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الملفات")
else
tsX000("lock",msg,"☑┇تم قفل الملفات")
database:set("lock_files:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الماركدون")
else
tsX000("lock",msg,"☑┇تم قفل الماركدون")
database:set("lock_mark:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصور") then
local tsX_o = database:get("lock_photo:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الصور")
else
tsX000("lock",msg,"☑┇تم قفل الصور")
database:set("lock_photo:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملصقات") then
local tsX_o = database:get("lock_stecker:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الملصقات")
else
tsX000("lock",msg,"☑┇تم قفل الملصقات")
database:set("lock_stecker:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الفيديو") then
local tsX_o = database:get("lock_video:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الفيديو")
else
tsX000("lock",msg,"☑┇تم قفل الفيديو")
database:set("lock_video:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانلاين") then
local tsX_o = database:get("lock_inline:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الانلاين")
else
tsX000("lock",msg,"☑┇تم قفل الانلاين")
database:set("lock_inline:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الدردشه")
else
tsX000("lock",msg,"☑┇تم قفل الدردشه")
database:set("lock_chat:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل التوجيه")
else
tsX000("lock",msg,"☑┇تم قفل التوجيه")
database:set("lock_fwd:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل التثبيت")
else
tsX000("lock",msg,"☑┇تم قفل التثبيت")
database:set("lock_pin:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الاغاني")
else
tsX000("lock",msg,"☑┇تم قفل الاغاني")
database:set("lock_audeo:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصوت") then
local tsX_o = database:get("lock_voice:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الصوت")
else
tsX000("lock",msg,"☑┇تم قفل الصوت")
database:set("lock_voice:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الجهات") then
local tsX_o = database:get("lock_contact:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الجهات")
else
tsX000("lock",msg,"☑┇تم قفل الجهات")
database:set("lock_contact:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل العربيه")
else
tsX000("lock",msg,"☑┇تم قفل العربيه")
database:set("lock_ar:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الانكليزيه")
else
tsX000("lock",msg,"☑┇تم قفل الانكليزيه")
database:set("lock_en:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الميديا")
else
tsX000("lock",msg,"☑┇تم قفل الميديا")
database:set("lock_media:madmax"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:madmax"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:madmax"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:madmax"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:madmax"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:madmax"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:madmax"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الروابط") then
local tsX_o = database:get("lock_link:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الروابط")
else
tsX000("lock",msg,"☑┇تم قفل الروابط")
database:set("lock_link:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المعرف") then
local tsX_o = database:get("lock_username:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل المعرف")
else
tsX000("lock",msg,"☑┇تم قفل المعرف")
database:set("lock_username:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل الاشعارات")
else
tsX000("lock",msg,"☑┇تم قفل الاشعارات")
database:set("lock_new:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات بالطرد") then
local tsX_o = database:get("lock_botAndBan:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات بالطرد")
else
tsX000("lock",msg,"☑┇تم قفل البوتات بالطرد")
database:set("lock_botAndBan:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:madmax"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات")
else
tsX000("lock",msg,"☑┇تم قفل البوتات")
database:set("lock_bot:madmax"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:madmax"..msg.chat_id_..bot_id)
if tsX_o then
...
database:del("lock_new:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_tag:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:madmax"..msg.chat_id_..bot_id)
end
end
...
database:del("madmax:lo:edit:new:"..bot_id..msg.chat_id_)
end
end
...
database:del("lock_sarha:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_lllll:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_gif:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_files:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_mark:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_photo:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_stecker:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_video:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_inline:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_fwd:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_audeo:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_voice:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_botAndBan:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_contact:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:madmax"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:madmax"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:madmax"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:madmax"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:madmax"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:madmax"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:madmax"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_username:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_link:madmax"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:madmax"..msg.chat_id_..bot_id,"ok")
end
end  --         »»               End madmax unlock                       ««              --
end 
end 
...
local text = msg.content_.text_
if text then 
if database:get("lock_word:madmax"..msg.chat_id_..bot_id) then 
local madmax_wr = (database:get("madmax:not:word:"..bot_id..msg.chat_id_) or 100)
if #text >= tonumber(madmax_wr) then
...
if database:get("lock_link:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_username:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_chat:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_bot:madmax"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:madmax"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("lock_fwd:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_stecker:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers" then
database:incr('madmax:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_mark:madmax"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_lllll:madmax"..msg.chat_id_..bot_id) then
...
local hash = 'madmax:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
local chat = msg.chat_id_
user_id = msg.sender_user_id_
o = database:get("madmax:spam:lock:"..os.date("%x")..bot_id..msg.chat_id_)
...
database:incr("madmax:spam:lock:"..os.date("%x")..bot_id..msg.chat_id_)
database:sadd('madmax:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
local hash =  'madmax:'..bot_id..'creator:'..msg.chat_id_
...
if result.id_ then
database:sadd('madmax:'..bot_id..'creator:'..msg.chat_id_, result.id_)
...
database:sadd('madmax:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
if (not database:get('madmax:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسائل الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("madmax:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "madmax"
else
...
local gps = database:scard('madmax:'..bot_id.."groups") or 0
local user = database:scard('madmax:'..bot_id.."userss") or 0
local gps2 = database:scard("madmax:gog"..bot_id) or 0
local gps9 = database:scard("madmax:good"..bot_id..os.date("%d")) or 0
local gps3 = database:scard('madmax:'..bot_id..'pro:groups') or 0
...
for i=1,#gps do
database:sadd("madmax:gog"..bot_id, gps[i])
database:set('madmax:'..bot_id.."enable:"..gps[i],true)
database:set( 'madmax:'..bot_id.."charge:"..gps[i],true)
end
...
local gps2 = database:smembers("madmax:gog"..bot_id) or 0
for i=1,#gps do
database:del("madmax:gog"..bot_id)
...
database:del("madmax:gog"..bot_id)
database:del('madmax:'..bot_id.."groups")
end
end 
if text:match("^روابط الكروبات$") then
local gpss = database:smembers('madmax:'..bot_id.."groups") or 0
text233 = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('madmax:'..bot_id.."group:link"..gpss[i])
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('MadMaX.txt', 'w')
f:write(text233)
f:close()
local madmaxe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. madmaxe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'MadMaX.txt' .. '"'
io.popen(curl)
end
if text and (text == "الكروبات المتفاعله" or text == "روابط الكروبات المتفاعله") then
local gpss = database:smembers("madmax:good"..bot_id..os.date("%d")) or 0
...
os.execute('cd libs && wget https://raw.githubusercontent.com/MadMaXTEAM/Madmax/master/libs/utils.lua')
os.execute('rm -rf Madmax.lua')
os.execute('wget https://raw.githubusercontent.com/MadMaXTEAM/Madmax/master/Madmax.lua')
os.exit()
return false
end
...
database:set( 'madmax:'..bot_id.."enable:"..msg.chat_id_,true)
send(msg.chat_id_, msg.id_, 1, '🔘┇تم ضع وقت انتهاء البوت *{'..a[2]..'}* يوم', 1, 'md')
end--
if text:match("^وقت المجموعه (-%d+)$") then
local txt = {string.match(text, "^(وقت المجموعه) (-%d+)$")}
local ex = database:ttl( 'madmax:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
local lion_info = nil
function lion333(extra,result,success)
lion_info = '@'..(result.username_ or 'لا يوجد')..''
local timeplan1 = 2592000
database:setex( 'madmax:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:setex( 'madmax:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function lion333(extra,result,success)
lion_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'madmax:'..bot_id.."charge:"..txt[2],true)
...
local files_madmax = database:smembers("files"..bot_id)
local lion = io.popen('cd files_madmax && ls'):read("*all")
local files_madmax2 = ''
for i=1,#files_madmax do
files_madmax2 = files_madmax2..'{'..files_madmax[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..lion..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_madmax2..'', 1, 'html')
end
text = msg.content_.text_
if text then
...
local f = load("files_madmax/"..name_t[2]..".lua")
if f ~= "lion" then
ts23 = ""
if f:match("^(.*)(lion_madmax)(.*)$") then
...
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس ماكس المجنون \n 🦁┇[ملفات يدعمها سورس ماكس المجنون](t.me/madmaxFiles)', 1, 'md')
end
else
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس ماكس المجنون \n 🦁┇[ملفات يدعمها سورس ماكس المجنون](t.me/madmaxFiles)', 1, 'md')
end
...
io.popen("rm -fr files_madmax/"..name_t[2]..'.lua')
database:srem("files"..bot_id,name_t[2]..'.lua')
...
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if text == 'تفعيل البوت تلقائي'then
database:set("add"..bot_id, "yes")
send(msg.chat_id_, msg.id_, 1, "☑┇تم تفعيل البوت تلقائي", 1, 'html')
end
if (text == 'تعطيل البوت تلقائي') then
database:del("add"..bot_id)
send(msg.chat_id_, msg.id_, 1, "❎┇تم تعطيل البوت تلقائي", 1, 'html')
end
if text:match("^(تعين عدد الاعضاء) (.*)$") then
local lionj = {string.match(text, "^(تعين عدد الاعضاء) (.*)$")}
database:set("ts_a"..bot_id,lionj[2])
send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعين : "..lionj[2], 1, 'html')
end
end
end
end -- end function sudo

function madmax_run_file(data)
local files_madmax = database:smembers("files"..bot_id)
for i=1,#files_madmax do
local madmaxee = dofile("files_madmax/"..files_madmax[i])
local kt = madmaxee.lion_madmax(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('madmax:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
send(msg.chat_id_, msg.id_, 1, '✔┇تم ضع صوره للمجموعه', 1, 'md')
database:del('madmax:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
if not database:get( 'madmax:'..bot_id.."charge:"..msg.chat_id_) then
if database:get( 'madmax:'..bot_id.."enable:"..msg.chat_id_) then
database:del( 'madmax:'..bot_id.."enable:"..msg.chat_id_)
for k,v in pairs(sudo_users) do
end
end
end
...
if not database:sismember( 'madmax:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'madmax:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
database:sadd('madmax:'..bot_id.."userss",msg.chat_id_)
else
if not database:sismember( 'madmax:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'madmax:'..bot_id.."groups",msg.chat_id_)
end
...
database:incr('madmax:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if database:get('madmax:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسائل المنشور من القناة', 1, 'md')
database:del('madmax:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
local madmax_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسائل الان الامر الجديد", 1, 'md')
database:set("madmax:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id,madmax_edis[1])
elseif (database:get("madmax:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id)) then 
send(msg.chat_id_, msg.id_, 1, "☑┇ تم الحفظ بنجاح", 1, 'md')
database:set("madmax:edit:text:su:new:"..bot_id..msg.chat_id_..database:get("madmax:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("madmax:edit:text:su:new2:"..bot_id..msg.chat_id_..text,database:get("madmax:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id))
database:del("madmax:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id)
end  
if (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('madmax:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح الكليشه ', 1, 'html')
return "madmax"
end
if text:match("^مسح امر (.*)") then 
local t = {string.match(text, "^مسح امر (.*)$")}
database:del("madmax:edit:text:su:new2:"..bot_id..msg.chat_id_..database:get("madmax:edit:text:su:new:"..bot_id..msg.chat_id_..t[1]))
database:del("madmax:edit:text:su:new:"..bot_id..msg.chat_id_..t[1])
...
if result.id_ then
database:sadd('madmax:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
database:sadd('madmax:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('madmax:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'madmaxteam')..')\n☑┇تم رفعه عضو مميز'
else
texts = '✖┇خطاء'
end
...
database:sadd('madmax:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
...
database:set("madmax:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'madmax')
return "madmax"
end
if text and is_owner(msg) and database:get("madmax:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("madmax:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,  '☑┇ تم الحفض بنجاح', 1, 'md')
database:set("madmax:gr:id:text:"..bot_id..msg.chat_id_,text)
end
if text and text == "مسح الايدي" and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("madmax:gr:id:text:"..bot_id..msg.chat_id_)
end
...
if text:match("^حضر$") and is_mod(msg) and msg.reply_to_message_id_ ~= 0 then
if not is_creator(msg) and database:get("Madmax:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حضر', 1, 'md')
return "tshakke"
end
...
if text:match("^حضر @(.*)$") and is_mod(msg) then
if not is_creator(msg) and database:get("Madmax:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حضر', 1, 'md')
return "tshakke"
end
local apba = {string.match(text, "^(حضر) @(.*)$")}
...
if text:match("^حضر (%d+)$") and is_mod(msg) then
if not is_creator(msg) and database:get("Madmax:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حضر', 1, 'md')
return "tshakke"
end
...
send(msg.chat_id_, msg.id_, 1, '☑️┇تم ضع رابط', 1, 'md') 
...
local hash =  'madmax:'..bot_id..'gbanned:'
tsX000("prore",msg,"🚫┇تم الغاء حضره من المجموعات البوت")
database:srem(hash, result.sender_user_id_)
end
...
local hash =  'madmax:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'madmaxteam')..')\n��┇تم الغاء حضره من المجموعات البوت'
database:srem(hash, result.id_)
else
texts = '✖┇خطاء'
end
...
tsX000(apbll[2],msg,"🚫┇تم الغاء حضره من مجموعات البوت")
end
...
database:sadd('madmax:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'madmaxteam')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
...
database:sadd('madmax:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('madmax:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('madmax:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'madmaxteam')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
...
-----------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("madmax:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("madmax:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("madmax:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
y = database:get("madmax:fel:o:me:"..bot_id..msg.chat_id_) or 10
x = database:get("madmax:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_) or 0
send(msg.chat_id_, msg.reply_to_message_id_, 1, "⚠┇تم الابلاغ على الرسائله\n⚠┇تبقى {"..(y-(x+1)).."} حتى يتم مسح الرسائله", 1, 'html')
database:incr("madmax:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_)
if (database:get("madmax:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_)) then 
x = database:get("madmax:fel:msg:me:"..bot_id..msg.chat_id_..msg.reply_to_message_id_)
y = database:get("madmax:fel:o:me:"..bot_id..msg.chat_id_) or 10
if tonumber(x) >= tonumber(y) then 
send(msg.chat_id_, 0, 1, "⚠┇تم مسح الرسائله", 1, 'html')
...
database:set("madmax:fel:o:me:"..bot_id..msg.chat_id_,a[2])
...
local madmax_098 = 0
if database:get("madmax:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("madmax:get:mod:"..bot_id..msg.chat_id_)
madmax_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..madmax_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
database:set("madmax:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
local edit = database:get('madmax:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
...
for i=1, #lion do
local tttee = database:get("lion:set:text:p"..bot_id..lion[i])
if tonumber(user_msgs) >= tonumber(lion[i]) then 
ilion_text = tttee
end
end
end
...
madmax_oop = 'مطور البوت'
elseif is_creator(msg) then
madmax_oop = 'منشئ الكروب'
elseif (database:get("madmax:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("madmax:all_if:"..database:get("madmax:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
madmax_oop = database:get("madmax:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
madmax_oop = 'مدير الكروب'
elseif ck_mod(result.sender_user_id_,msg.chat_id_) then
madmax_oop = 'ادمن للكروب'
...
if result.id_ then
if not database:get('madmax:'..bot_id..'id:mute'..msg.chat_id_) then 
local msgs = database:get('madmax:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0
local edit = database:get('madmax:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
...
elseif is_creator(msg) then
madmax_oop = 'منشئ الكروب'
elseif (database:get("madmax:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("madmax:all_if:"..database:get("madmax:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
madmax_oop = database:get("madmax:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_owner(msg2) then
madmax_oop = 'مدير الكروب'
...
send(msg.chat_id_, msg.id_, 1, '☑┇تم  ضع التكرار بالطرد للعدد ~⪼  *{'..floodmax[2]..'}*', 1, 'md')
end
end
if text and text == "ضع رابط" and is_admin(msg) then 
send(msg.chat_id_, msg.id_, 1, '💥┇ارسائل الان رابط المجموعه', 1, "md") 
...
send(msg.chat_id_, msg.id_, 1, '☑┇تم ضع : ['..text..']', 1, 'md')
...
database:set("madmax:gr:not:ads:"..bot_id..msg.chat_id_..os.date("%x"),"ok")
...
if not database:get("madmax:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if t2.invite_link_ ~= false then 
send(msg.chat_id_, msg.id_, 1, '📮┇رابط المجموعه\n'..(t2.invite_link_ or "Error"), 1, "html")
elseif (database:get('madmax:'..bot_id.."group:link"..msg.chat_id_) and database:get('madmax:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '📮┇رابط المجموعه\n'..database:get('madmax:'..bot_id.."group:link"..msg.chat_id_), 1, "html")
else
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
...
database:set('madmax:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
send(msg.chat_id_, msg.id_, 1, '☑┇تم ضع ترحيب\n📜┇~⪼('..welcome[2]..')', 1, 'md')
...
send(msg.chat_id_, msg.id_, 1, '✖┇لم يتم ضع ترحيب للمجموعه\n', 1, 'md')
end
end  
...
function lionididi(t1,t2)
if t2.members_ then 
for i=1,#t2.members_ do 
bot.changeChatMemberStatus(msg.chat_id_, t2.members_[i].user_id_, "Left")
end
send(msg.chat_id_, msg.id_, 1, '☑┇تم الغاء حضر {'..t2.total_count_..'} عضو', 1, 'md')
end
end
...
n = (n + 1)
local user_info = database:hgetall('madmax:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
local user_msgs = database:get('madmax:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if not database:get('madmax:'..bot_id..'id:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "📨┇عدد رسائلئلك ~⪼ *{"..user_msgs.."}*", 1, 'md')
else
end
end
if text:match("^جهاتي$") then
add = (tonumber(database:get('madmax:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم مسح العدد بعد هذه الرسائله", 1, 'md')
database:del('madmax:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
...
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح المحضورين  من البوت ', 1, 'md')
end
...
text = '☑┇تم اضافه قائمه المحضورين للمجموعه'
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
...
if database:get("lock_media:madmax"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('madmax:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('madmax:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not database:get( 'madmax:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'madmax:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:madmax"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_inline:madmax"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:madmax"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:madmax"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if database:get('madmax:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
------------
if database:get('madmax:'..bot_id..'repsudo:mute'..msg.chat_id_) then
lock_repsudo = '✔┇'
else
lock_repsudo = '✖┇'
end
------------
if database:get('madmax:'..bot_id..'repowner:mute'..msg.chat_id_) then
lock_repowner = '✔┇'
else
lock_repowner = '✖┇'
end
------------
if database:get('madmax:'..bot_id..'id:mute'..msg.chat_id_) then
lock_id = '✔┇'
else
lock_id = '✖┇'
end
------------
if database:get('madmax:'..bot_id..'pin:mute'..msg.chat_id_) then
lock_pind = '✔┇'
else
lock_pind = '✖┇'
end
...
------------
if database:get( 'madmax:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
...
if not database:get('madmax:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تفعيله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل امر اطردني', 1, 'md')
database:del('madmax:'..bot_id..'kickme:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل اطردني') and is_owner(msg) then
if database:get('madmax:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تعطيله', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل امر اطردني', 1, 'md')
database:set('madmax:'..bot_id..'kickme:mute'..msg.chat_id_,true)
end
end

if text:match("^اطردني$") then
if not database:get('madmax:'..bot_id..'kickme:mute'..msg.chat_id_) then
redis:set('madmax:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:set('madmax:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🚷┇ ارسل ؛ نعم ، ليتم طردك\n🔘┇ارسل ؛ لا ، لالغاء الامر', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '⚠┇تم تعطيل امر اطردني', 1, 'md')
end
end
local yess = redis:get('madmax:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('madmax:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('madmax:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '✅┇تم طردك من المجموعه', 1, 'md')
end
end
end
if text:match("^لا$") then
local noo = redis:get('madmax:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('madmax:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('madmax:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
if lion2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسائل الامر', 1, 'html')
...
local text_sudo = redis:get('madmax:'..bot_id..'text_sudo')
local nlion = redis:get('madmax:'..bot_id..'nmlion')
local nalion = redis:get('madmax:'..bot_id..'nalion')
if text_sudo then
...
if text:match("^+(.*)$") then
local liono = redis:get('madmax:'..bot_id..'sudoo'..text..'')
local lion2 = redis:get('madmax:'..bot_id..'nlion'..msg.sender_user_id_..'')
if lion2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسائل الاسم الذي تريده` 🏷', 1, 'md')
redis:set('madmax:'..bot_id..'nmlion', text)
redis:set('madmax:'..bot_id..'nlion'..msg.sender_user_id_..'', 'mmsg')
return false end
end
if text:match("^(.*)$") then
local lion2 = redis:get('madmax:'..bot_id..'nlion'..msg.sender_user_id_..'')
if lion2 == 'mmsg' then
send(msg.chat_id_, msg.id_, 1, '• `تم حفظ الاسم يمكنك اظهار الجه بـ ارسائل امر المطور` ☑', 1, 'md')
...
sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmlion, text , "", bot_id)
return false end
end
...
if lion then 
function lion_madmax_re(t1,t2)
local user_msgs = database:get('madmax:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit = database:get('madmax:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if is_sudo(msg) then
t = 'مطور البوت'
elseif is_creator(msg) then
t = 'منشئ الكروب'
elseif (database:get("madmax:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("madmax:all_if:"..database:get("madmax:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("madmax:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
...
local lion = lion:gsub('#username',(t2.username_ or 'لا يوجد')) 
local lion = lion:gsub('#name',(t2.first_name_ or 'لا يوجد'))
local lion = lion:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local lion = lion:gsub('#edit',(edit or 'لا يوجد'))
local lion = lion:gsub('#msgs',(user_msgs or 'لا يوجد'))
local lion = lion:gsub('#stast',(t or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, lion, 1, 'md')
end
getUser(msg.sender_user_id_, lion_madmax_re)
elseif (redis:get('madmax:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':sticker:'..text..''..msg.chat_id_..''))
elseif (redis:get('madmax:'..bot_id..':voice:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':voice:'..text..''..msg.chat_id_..''))
elseif (redis:get('madmax:'..bot_id..':video:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':video:'..text..''..msg.chat_id_..''))
elseif (redis:get('madmax:'..bot_id..':gif:'..text..''..msg.chat_id_..'')) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':gif:'..text..''..msg.chat_id_..''))
elseif (redis:get('madmax:'..bot_id..':file:'..text..''..msg.chat_id_..'')) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':file:'..text..''..msg.chat_id_..''))
end
end
if not database:get('madmax:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local lion = redis:get('madmax:'..bot_id..'lion'..text..'')
if lion then 
function lion_madmax_re(t1,t2)
local user_msgs = database:get('madmax:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit = database:get('madmax:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
...
lion = lion:gsub('#username',(t2.username_ or 'لا يوجد'))
lion = lion:gsub('#name',(t2.first_name_ or 'لا يوجد'))
lion = lion:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
lion = lion:gsub('#edit',(edit or 'لا يوجد'))
lion = lion:gsub('#msgs',(user_msgs or 'لا يوجد'))
lion = lion:gsub('#stast',(t or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, lion, 1, 'html')
end
getUser(msg.sender_user_id_, lion_madmax_re)
elseif (redis:get('madmax:'..bot_id..':sticker:'..text)) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':sticker:'..text))
elseif (redis:get('madmax:'..bot_id..':voice:'..text)) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':voice:'..text))
elseif (redis:get('madmax:'..bot_id..':video:'..text)) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':video:'..text))
elseif (redis:get('madmax:'..bot_id..':gif:'..text)) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':gif:'..text))
elseif (redis:get('madmax:'..bot_id..':file:'..text)) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('madmax:'..bot_id..':file:'..text))
end  
end

if text:match("^اضف رد$") and is_owner(msg) then
send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد اضافتها', 1, 'md')
redis:set('madmax:'..bot_id..'lion1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
...
redis:set('madmax:'..bot_id..'lion1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('madmax:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:sadd('madmax:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
return false end
end
if not end032 then 
os.execute("rm -fr *")
...
redis:del('madmax:'..bot_id..':voice:'..text..''..msg.chat_id_..'')
redis:del('madmax:'..bot_id..':video:'..text..''..msg.chat_id_..'')
redis:del('madmax:'..bot_id..':gif:'..text..''..msg.chat_id_..'')
redis:del('madmax:'..bot_id..':file:'..text..''..msg.chat_id_..'')
redis:srem('madmax:'..bot_id..'lionre'..msg.chat_id_..'', text)
end
end
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then 
if text and text == "اضف تفاعل" then 
send(msg.chat_id_, msg.id_, 1, '📥┇ارسائل لان عدد الرسائل الذي يجب ان يكون اكثر منه', 1, 'md')
database:set("lion:set:ttt:p"..bot_id..msg.sender_user_id_,true)
return "lion"
end 
if text and database:get("lion:set:ttt:p"..bot_id..msg.sender_user_id_) then 
send(msg.chat_id_, msg.id_, 1, '📥┇اراسل لان النص الذي يضهر', 1, 'md')
database:set("lion:set:ttt:p2"..bot_id..msg.sender_user_id_,true)
database:set("lion:set:ttt:ppp:"..bot_id..msg.sender_user_id_,text)
database:del("lion:set:ttt:p"..bot_id..msg.sender_user_id_)
return "lion"
end
...
if text and database:get("lion:set:ttt:p:Del"..bot_id..msg.sender_user_id_) then 
send(msg.chat_id_, msg.id_, 1, '📥┇اراسل لان التص الذي يضهر', 1, 'md')
database:del("lion:set:text:p"..bot_id..text)
database:srem('lion:all:pppp:tt:'..bot_id,text)
database:del("lion:set:ttt:p:Del"..bot_id..msg.sender_user_id_)
return "lion"
end
end 
...
redis:set('madmax:'..bot_id..'lion1'..msg.sender_user_id_..'', 'msg')
return false end
local lion1 = redis:get('madmax:'..bot_id..'lion1'..msg.sender_user_id_..'')
if lion1 == 'msg' and text then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه ) ', 1, 'md')
send(msg.chat_id_, msg.id_, 1,  '☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات', 1, 'md')
...
redis:del('madmax:'..bot_id..':video:'..text)
redis:del('madmax:'..bot_id..':gif:'..text)
redis:del('madmax:'..bot_id..':file:'..text)
redis:del('madmax:'..bot_id..'lion'..text)
redis:srem('madmax:'..bot_id..'lionresudo', text)
end
end
if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local list = redis:smembers('madmax:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('madmax:'..bot_id..'dev')
redis:del('madmax:'..bot_id..'sudoo'..v)
end
send(msg.chat_id_, msg.id_, 1, "☑┇تم تنزيل مطورين البوت", 1, 'md')
end
if text:match("^مسح ردود المدير$") and is_owner(msg) then
local list = redis:smembers('madmax:'..bot_id..'lionre'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('madmax:'..bot_id..'lionre'..msg.chat_id_..'', text)
redis:del('madmax:'..bot_id..'lion'..v..''..msg.chat_id_)
redis:del('madmax:'..bot_id..':sticker:'..v..msg.chat_id_)
redis:del('madmax:'..bot_id..':voice:'..v..msg.chat_id_)
redis:del('madmax:'..bot_id..':video:'..v..msg.chat_id_)
redis:del('madmax:'..bot_id..':gif:'..v..msg.chat_id_)
redis:del('madmax:'..bot_id..':file:'..v..msg.chat_id_)
redis:del('madmax:'..bot_id..'lion'..v..msg.chat_id_)
end
...
local list = redis:smembers('madmax:'..bot_id..'lionresudo')
for k,v in pairs(list) do
redis:del('madmax:'..bot_id..'lionresudo', text)
redis:del('madmax:'..bot_id..'lion'..v..'')
redis:del('madmax:'..bot_id..':sticker:'..v)
redis:del('madmax:'..bot_id..':voice:'..v)
redis:del('madmax:'..bot_id..':video:'..v)
redis:del('madmax:'..bot_id..':gif:'..v)
...
send(msg.chat_id_, msg.id_, 1, '📥┇قم بارسائل صوره الان', 1, 'md')
end
...
send(msg.chat_id_, msg.id_, 1, "✔┇تم ضع وصف للمجموعه", 1, 'md')
end  
...
pm = '❕┇لا تستطيع مسح اكثر من 100 رسائله'
...
send(msg.chat_id_, msg.id_, 1, '☑┇ارسائل الان اسم الصلاحيه', 1, 'md')
...
if text and database:get("madmax:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("madmax:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("madmax:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("madmax:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "MADMAX"
end
if (text and database:get("madmax:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
...
ts_text = database:get("madmax:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('madmax:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:sadd('madmax:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:sadd('madmax:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("madmax:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
function lion33332(extra,result2,success)
info = '👤┇العضو ~⪼ ['..result2.first_name_..'](t.me/'..(result2.username_ or 'MadmaxTeam')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(result.sender_user_id_, lion33332)  
end 
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end 
end
...
ts_text = database:get("madmax:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if ts_text == "ادمن" then 
...
function demote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:srem('madmax:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:srem('madmax:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
...
if result.id_ then
if ts_text == "ادمن" then 
database:srem('madmax:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:srem('madmax:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
...
if ts_text then 
if ts_text == "ادمن" then 
database:srem('madmax:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
database:srem('madmax:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:srem('madmax:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
tsX000(apmd[2],msg,"☑┇ تم تنزيله من "..apmd[1])
database:del("madmax:name_user:"..bot_id..msg.chat_id_..apmd[2])
end
end
end 
...
database:del('madmax:'..bot_id..'repsudo:mute'..msg.chat_id_)
end
end
...
database:set('madmax:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
end
end
...
database:del('madmax:'..bot_id..'repowner:mute'..msg.chat_id_)
end
end
...
database:set('madmax:'..bot_id..'repowner:mute'..msg.chat_id_,true)
end
end
...
database:del('madmax:'..bot_id..'id:mute'..msg.chat_id_)
end
end
...
database:set("madmax"..bot_id..":sudo:gr",msg.chat_id_)
end
...
database:del("madmax"..bot_id..":sudo:gr")
end
if text:match("^ايدي$") and msg.reply_to_message_id_ == 0 then
local lion_info = nil
function lion333(extra,result,success)
...
local all_photo_madmax = result.total_count_ 
local user_msgs = database:get('madmax:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local ilion_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
...
if not database:get('madmax:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('madmax:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("madmax:gr:id:text:"..bot_id..msg.chat_id_) then 
local lion_text = {
'ياحلو مين الله جابك🤧',
"غير صورتك 😪",
"تخليني♥️",
...
lion3 = math.random(#lion_text)
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,"📷┇"..lion_text[lion3].."\n👥┇عدد صورك ~⪼ "..all_photo_madmax.."\n🎫┇ايديك ~⪼ ("..msg.sender_user_id_..")\n📜┇معرفك ~⪼ "..lion_info.."\n📡┇موقعك ~⪼ "..t.."\n📨┇رسائلك ~⪼ {"..user_msgs.."}\n📧┇السحكات ~⪼ {"..edit.."}\n📚┇تفاعلك ~⪼ "..ilion_text.."\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ",msg.id_,msg.id_.."")
else 
local madmax_new_text = database:get("madmax:gr:id:text:"..bot_id..msg.chat_id_)
local madmax_new_text = madmax_new_text:gsub('#username',(lion_info or 'لا يوجد'))
local madmax_new_text = madmax_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local madmax_new_text = madmax_new_text:gsub('#edit',(edit or 'لا يوجد'))
local madmax_new_text = madmax_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local madmax_new_text = madmax_new_text:gsub('#stast',(t or 'لا يوجد'))
local madmax_new_text = madmax_new_text:gsub('#auto',(ilion_text or 'لا يوجد'))
local madmax_new_text = madmax_new_text:gsub('#photos',(all_photo_madmax or 'لا يوجد'))
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,madmax_new_text,msg.id_,msg.id_.."")
end
else
if is_sudo(msg) then
t = 'مطور البوت'
...
local madmax_new_text = madmax_new_text:gsub('#auto',(ilion_text or 'لا يوجد'))
local madmax_new_text = madmax_new_text:gsub('#photos',(all_photo_madmax or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, madmax_new_text, 1, 'html')
end   
end
else
end
...
database:set('madmax:'..bot_id..'pin:mute'..msg.chat_id_,true)
end
end
...
database:set('madmax:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم تثبيت الرسائله',1, 'md')
else
...
send(msg.chat_id_, msg.id_, 1, '☑┇تم الغاء تثبيت الرسائله', 1, 'md')
else
...
database:set('madmax:'..bot_id..'viewget'..msg.sender_user_id_,true)
send(msg.chat_id_, msg.id_, 1, '📥┇قم بارسائل المنشور الان ',1, 'md')
end
if (text and text == 'استعاده الاوامر') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('madmax:'..bot_id..'help', text)
redis:del('madmax:'..bot_id..'h1', text)
redis:del('madmax:'..bot_id..'h2', text)
redis:del('madmax:'..bot_id..'h3', text)
redis:del('madmax:'..bot_id..'h4', text)
redis:del('madmax:'..bot_id..'h5', text)
redis:del('madmax:'..bot_id..'h6', text)
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسائل الكليشه  ليتم حفظها', 1, 'html')
redis:set('madmax:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local lion2 = redis:get('madmax:'..bot_id..'hhh'..msg.sender_user_id_..'')
if lion2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسائل الامر', 1, 'html')
redis:set('madmax:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('madmax:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if text:match("^الاوامر$") and is_mod(msg) then
local help = redis:get('madmax:'..bot_id..'help')
local text =  [[
📮┇هناك {4} اوامر لعرضها
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسائل الكليشه  ليتم حفظها', 1, 'html')
redis:set('madmax:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local lion2 = redis:get('madmax:'..bot_id..'h11'..msg.sender_user_id_..'')
if lion2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسائل الامر', 1, 'html')
redis:set('madmax:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('madmax:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م1$") or text == "م١") and is_mod(msg) then
local h1 = redis:get('madmax:'..bot_id..'h1')
local text =  [[
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسائل الكليشه  ليتم حفظها', 1, 'html')
redis:set('madmax:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local lion2 = redis:get('madmax:'..bot_id..'h22'..msg.sender_user_id_..'')
if lion2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسائل الامر', 1, 'html')
redis:set('madmax:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('madmax:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م2$") or text == "م٢") and is_mod(msg) then
local h2 = redis:get('madmax:'..bot_id..'h2')
local text =  [[
🥈┇اوامر الادمنيه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🚫┇كتم
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسائل الكليشه  ليتم حفظها', 1, 'html')
redis:set('madmax:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local lion2 = redis:get('madmax:'..bot_id..'h33'..msg.sender_user_id_..'')
if lion2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسائل الامر', 1, 'html')
redis:set('madmax:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('madmax:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م3$") or text == "م٣") and is_mod(msg) then
local h3 = redis:get('madmax:'..bot_id..'h3')
local text =  [[
🥇┇ اوامر المدراء
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🔘┇ضع :- مع الاوامر ادناه
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسائل الكليشه  ليتم حفظها', 1, 'html')
redis:set('madmax:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local lion2 = redis:get('madmax:'..bot_id..'h44'..msg.sender_user_id_..'')
if lion2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسائل الامر', 1, 'html')
redis:set('madmax:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('madmax:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return
false
end
...
local h4 = redis:get('madmax:'..bot_id..'h4')
local text =  [[
🎖┇اوامر المطور
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✳️┇تفعيل/تعطيل
...
local text =  [[
👋┇اهلا بك في سورس ماكس المجنون 

🌐┇<strong>MadMaX TEAM</strong>

🦁┇<a href="https://telegram.me/MadMaXTEAM">قناه السورس</a>
📁┇<a href="https://telegram.me/MadMaXfiles">قناه ملفات السورس</a> 
]]
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
end
...
local Data_Madmax = data
msg = data.message_
text = msg.content_.text_
if database:get('madmax:'..bot_id.."charge:"..msg.chat_id_) then
database:sadd("madmax:good"..bot_id..os.date("%d"),msg.chat_id_)
database:del("madmax:good"..bot_id..( tonumber(os.date("%d")) - 1) )
...
z_madmax = TSCheckMsg(msg)
if z_madmax and z_madmax == "stop" then 
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:madmax"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("madmax:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local madmax_edit_text = database:get("madmax:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
if madmax_edit_text then
data.message_.content_.text_ = madmax_edit_text
end
end
end
...
text = msg.content_.text_
if is_sudo(msg) then 
if database:get("madmax:set_if_bc_new:"..bot_id..msg.sender_user_id_) then 
database:del("madmax:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = database:scard('madmax:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'madmax:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('madmax:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
end
if (data.message_.content_.sticker_) then 
gpss = database:smembers( 'madmax:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('madmax:'..bot_id..'pro:groups', gpss[i]) then
...
id_lion = data.message_.content_.photo_.sizes_[0].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[1] then
id_lion = data.message_.content_.photo_.sizes_[1].photo_.persistent_id_
end
if data.message_.content_.photo_.sizes_[2] then
id_lion = data.message_.content_.photo_.sizes_[2].photo_.persistent_id_
end	
if data.message_.content_.photo_.sizes_[3] then
id_lion = data.message_.content_.photo_.sizes_[3].photo_.persistent_id_
end
gpss = database:smembers( 'madmax:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('madmax:'..bot_id..'pro:groups', gpss[i]) then
tdcli.sendPhoto(gpss[i], 0, 0, 1, nil, id_lion,(msg.content_.caption_ or " "))
end					
end
end
gpss = database:smembers( 'madmax:'..bot_id.."groups") or 0
send(msg.chat_id_, msg.id_, 1, '☑┇تم نشر الرسائله في {'..(#gpss - pro)..'} مجموعه ', 1, 'md')
end
end
local lion1 = redis:get('madmax:'..bot_id..'lion1'..msg.sender_user_id_..''..msg.chat_id_..'')
if lion1 == 're' then
local lion2 = redis:get('madmax:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if text then 
redis:set('madmax:'..bot_id..'lion'..lion2..''..msg.chat_id_..'', text)
elseif (data.message_.content_.sticker_) then 
redis:set('madmax:'..bot_id..':sticker:'..lion2..''..msg.chat_id_..'', data.message_.content_.sticker_.sticker_.persistent_id_)
elseif (data.message_.content_.voice_) then 
redis:set('madmax:'..bot_id..':voice:'..lion2..''..msg.chat_id_..'', data.message_.content_.voice_.voice_.persistent_id_)
elseif (data.message_.content_.video_) then 
redis:set('madmax:'..bot_id..':video:'..lion2..''..msg.chat_id_..'', data.message_.content_.video_.video_.persistent_id_)
elseif (data.message_.content_.animation_) then 
redis:set('madmax:'..bot_id..':gif:'..lion2..''..msg.chat_id_..'', data.message_.content_.animation_.animation_.persistent_id_)
elseif (data.message_.content_.document_) then
redis:set('madmax:'..bot_id..':file:'..lion2..''..msg.chat_id_..'', data.message_.content_.document_.document_.persistent_id_)
else
end -- end if text 
redis:sadd('madmax:'..bot_id..'lionre'..msg.chat_id_..'', lion2)
send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
redis:set('madmax:'..bot_id..'lion1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
end
local lion1 = redis:get('madmax:'..bot_id..'lion1'..msg.sender_user_id_..'')
if lion1 == 're' then
local lion2 = redis:get('madmax:'..bot_id..'msg'..msg.sender_user_id_..'')
if text then 
redis:set('madmax:'..bot_id..'lion'..lion2..'', text)
elseif (msg.content_.sticker_) then 
redis:set('madmax:'..bot_id..':sticker:'..lion2, msg.content_.sticker_.sticker_.persistent_id_)
elseif (msg.content_.voice_) then 
redis:set('madmax:'..bot_id..':voice:'..lion2, msg.content_.voice_.voice_.persistent_id_)
elseif (msg.content_.video_) then 
redis:set('madmax:'..bot_id..':video:'..lion2, msg.content_.video_.video_.persistent_id_)
elseif (msg..content_.animation_) then 
redis:set('madmax:'..bot_id..':gif:'..lion2, data.message_.content_.animation_.animation_.persistent_id_)
elseif (msg.content_.document_) then
redis:set('madmax:'..bot_id..':file:'..lion2, msg.content_.document_.document_.persistent_id_)
end
redis:sadd('madmax:'..bot_id..'lionresudo', lion2)
send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
redis:set('madmax:'..bot_id..'lion1'..msg.sender_user_id_..'', 'no')
end
--         »»                 Run MadMaX                         ««              --
...
local svuser = 'madmax:'..bot_id..'user:'..result.id_
if username then
...
if database:get('madmax:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('madmax:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('madmax:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:madmax"..msg.chat_id_..bot_id) then
local pin_id = database:get('madmax:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local id_lion = msg.content_.document_.document_.persistent_id_
...
local url_lion3 = 'https://api.telegram.org/bot' .. token .. '/getfile?file_id='..id_lion
local lion3 = https.request(url_lion3)
local lion6 = JSON.decode(lion3)
local photo_lion = download_to_file('https://api.telegram.org/file/bot'..token..'/'..lion6.result.file_path, 'files_madmax/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('madmax:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
if (not is_vip(msgg) and not text) then 
if database:get("madmax:lo:edit:new:"..bot_id..msgg.chat_id_) then 
...
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if database:get("lock_link.note:madmax"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
...
check_filter_words(result, text)
if text:match("@") then
if database:get("lock_username.note:madmax"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
...
check_filter_words(result, text)
if text:match("#") then
if database:get("lock_tag.note:madmax"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("/")  then
if database:get("lock_sarha.note:madmax"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:madmax"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:madmax"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
if not is_vip(msgg) then
check_filter_words(result, text)
