...
local function keko_MAX(data)
    ...
    database:set("MAX:loock:add:w:"..msg.chat_id_..bot_id,"MAX")
    send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعطيل خاصيه ضافني", 1, 'html')
    return "keko"
    end 
    if text and text == "تفعيل ضافني" then 
    database:del("MAX:loock:add:w:"..msg.chat_id_..bot_id)
    send(msg.chat_id_, msg.id_, 1, "☑️┇ تم تفعيل خاصيه ضافني", 1, 'html')
    return "keko"
    end 
    end
    if not database:get("MAX:loock:add:w:"..msg.chat_id_..bot_id) then 
    if msg.content_.ID == "MessageChatAddMembers" then
    database:set("MAX:add:me:w:"..bot_id..msg.chat_id_..msg.content_.members_[0].id_,msg.sender_user_id_)    
    end 
    if text and text:match("(.*)(ضافني)(.*)") then 
    if_keko =  database:get("MAX:add:me:w:"..bot_id..msg.chat_id_..msg.sender_user_id_)    
    if not if_keko then 
